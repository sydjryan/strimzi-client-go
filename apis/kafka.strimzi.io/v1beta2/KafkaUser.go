...
type KafkaUserMetadata map[string]string
...
// UnmarshalJSON implements json.Unmarshaler.
func (j *KafkaUserSpecQuotas) UnmarshalJSON(b []byte) error {
	type Plain KafkaUserSpecQuotas
	var plain Plain
	if err := json.Unmarshal(b, &plain); err != nil {
		return err
	}
	if plain.ConsumerByteRate != nil && 0 > *plain.ConsumerByteRate {
		return fmt.Errorf("field %s: must be >= %v", "consumerByteRate", 0)
	}
	if plain.ControllerMutationRate != nil && 0 > *plain.ControllerMutationRate {
		return fmt.Errorf("field %s: must be >= %v", "controllerMutationRate", 0)
	}
	if plain.ProducerByteRate != nil && 0 > *plain.ProducerByteRate {
		return fmt.Errorf("field %s: must be >= %v", "producerByteRate", 0)
	}
	if plain.RequestPercentage != nil && 0 > *plain.RequestPercentage {
		return fmt.Errorf("field %s: must be >= %v", "requestPercentage", 0)
	}
	*j = KafkaUserSpecQuotas(plain)
	return nil
}
